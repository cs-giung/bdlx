...
from bdlx.sgmcmc import sgrhmc_diagef
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--damping_factor', default=0.0, type=float,
        help='damping factor to faciliate inversion (default: 0.0)')
    parser.add_argument(
    ...
    def perex_log_likelihood_fn(param, batch):
        # pylint: disable=redefined-outer-name
        """Computes log likelihood."""
        logits = forward_fn(param, batch['inputs'][None])
        target = batch['target'][None]
        log_likelihood = jnp.sum(
            target * jax.nn.log_softmax(logits), axis=-1)[0]
        aux = OrderedDict({'log_likelihood': log_likelihood})
        return log_likelihood, aux

    def perex_log_prior_fn(param, batch):
        # pylint: disable=redefined-outer-name,unused-argument
        """Computes unnormalized log prior."""
        log_prior = jnp.negative(0.5 * sum(
            jnp.sum(e**2) / args.prior_variance
            for e in jax.tree_util.tree_leaves(param))) / trn_dataset_size
        aux = OrderedDict({'log_prior': log_prior})
        return log_prior, aux

    @partial(jax.pmap, axis_name='batch')
    def update_fn(state, batch, step_size):
        # pylint: disable=redefined-outer-name
        """Updates state."""
        aux, state = sgrhmc_diagef.step(
            state=state,
            batch=batch,
            num_train=trn_dataset_size,
            perex_log_likelihood_fn=perex_log_likelihood_fn,
            perex_log_prior_fn=perex_log_prior_fn,
            step_size=step_size,
    ...
    state = sgrhmc_diagef.SGRHMCDiagEFState(
        step=0, rng_key=jax.random.PRNGKey(args.seed), position=init_position,
        momentum=init_momentum, momentum_nu=init_momentum_nu)
    ...
    for sample_idx in range(1, args.num_samples + 1):
        ...
        for update_idx in range(1, args.num_updates + 1):
            ...
            if update_idx == 1 or update_idx % 5000 == 0:
                ...
                if jnp.isnan(summarized['trn/log_likelihood']):
                    break
