...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--smoothing_mu', default=0.9, type=float,
        help='smoothing factor (default: 0.9)')
    parser.add_argument(
        '--smoothing_nu', default=0.999, type=float,
        help='smoothing factor (default: 0.999)')
    parser.add_argument(
    ...
    def update_fn(state, batch, step_size):
        # pylint: disable=redefined-outer-name
        """Updates state."""
        aux, state = asgld.step(
            state=state,
            batch=batch,
            energy_fn=energy_fn,
            step_size=step_size,
            smoothing=(args.smoothing_mu, args.smoothing_nu),
            bias=args.bias,
