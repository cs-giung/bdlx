...
from examples.imagenet_ft import image_processing
from examples.imagenet_ft.default import get_args, str2bool
from examples.imagenet_ft.input_pipeline import \
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        help='build model defined in transformerx (required)')
    parser.add_argument(
    ...
    parser.add_argument(
        '--optim_lr', default=0.001, type=float,
    ...
    parser.add_argument(
        '--momentum_nu', default=0.999, type=float,
    ...
    args, print_fn, time_stamp = get_args(
        ...
        libraries=(jax, jaxlib, tensorflow, tensorflow_datasets))
    ...
    if args.model_name in (
            ...
            'cs-giung/vit-large-patch16-imagenet21k-augreg',
        ):
        from transformerx.models.vit.default import \
            load_jx_config, load_jx_params
        from transformerx.models.vit.modeling import \
            forward_fn as _forward_fn, ViTInputs as Inputs
        config = load_jx_config(args.model_name)
        qarams = load_jx_params(args.model_name)
        qarams.pop('head')
        hidden_size = config.hidden_size
        pixel_m = np.array([0.5, 0.5, 0.5])
        pixel_s = np.array([0.5, 0.5, 0.5])

    elif args.model_name in (
            'cs-giung/convnext-v1-base-imagenet21k',
            'cs-giung/convnext-v1-large-imagenet21k',
        ):
        from transformerx.models.convnext.default import \
            load_jx_config, load_jx_params
        from transformerx.models.convnext.modeling import \
            forward_fn as _forward_fn, ConvNextInputs as Inputs
        config = load_jx_config(args.model_name)
        qarams = load_jx_params(args.model_name)
        qarams.pop('head')
    ...
    init_position = {}
    for k in ['layers', 'post_layernorm']:
        init_position[k] = qarams.pop(k)

    init_position = {
        'ext': init_position,
        'cls': {
            'kernel': jax.random.normal(
                jax.random.PRNGKey(args.seed), (hidden_size, num_classes)),
            'bias': jnp.zeros((num_classes,))}}

    if args.model_ckpt:
        init_position = load(args.model_ckpt).position
    ...
    def forward_fn(params, inputs):
        ...
        inputs = Inputs(input_pixels=inputs)

        logits = _forward_fn(
            {**params['ext'], **qarams}, inputs, config).last_hidden_states
